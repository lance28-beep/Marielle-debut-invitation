...
function MessageForm({ onSuccess, onMessageSent }: MessageFormProps) {
  ...
  const handleSubmit = async (e: React.FormEvent<HTMLFormElement>) => {
    ...
    try {
      await fetch(
        "https://docs.google.com/forms/d/e/1FAIpQLSdcScOom5JJqRJvg-W7BktTVZnU6BadfnsSAocwQTNrgK1OAg/formResponse",
        {
          method: "POST",
          mode: "no-cors",
          body: googleFormData,
  ...
  return (
    <div className="relative w-full max-w-md mx-auto px-3 sm:px-0">
      ...
      {/* Decorative background elements (moonlit blues) */}
      <div className="absolute -top-3 -left-3 w-8 h-8 bg-[#7db7ff]/20 rounded-full blur-sm animate-pulse-slow" />
      <div className="absolute -bottom-4 -right-4 w-12 h-12 bg-[#cfe7ff]/25 rounded-full blur-md animate-pulse-slow" />
      
      <Card className={`relative w-full border-2 border-[#9ac8ff]/70 shadow-[0_12px_30px_rgba(0,0,0,0.35)] bg-white/60 backdrop-blur-md transition-all duration-500 group overflow-hidden rounded-2xl ${
        isFocused ? 'scale-[1.01] border-[#7db7ff] bg-white/75' : 'hover:bg-white/70'
      } ${isSubmitted ? 'animate-bounce' : ''}`}>
        {/* Glass effect gradient overlays */}
        <div className="absolute inset-0 bg-gradient-to-br from-[#7db7ff]/18 via-white/10 to-transparent" />
        <div className="absolute inset-0 bg-gradient-to-t from-white/28 via-transparent to-transparent"></div>
        
        {/* Frosted glass effect */}
        <div className="absolute inset-0 backdrop-blur-xl bg-gradient-to-br from-white/18 to-white/6" />
        
        {/* Animated shine effect */}
        <div className="absolute inset-0 bg-gradient-to-r from-transparent via-[#bfe0ff]/28 to-transparent -translate-x-full group-hover:translate-x-full transition-transform duration-1000" />
        
        {/* Success animation overlay */}
        {isSubmitted && (
          <div className="absolute inset-0 bg-gradient-to-br from-[#0f2541]/35 via-[#7db7ff]/20 to-transparent flex items-center justify-center z-20 pointer-events-none">
            <div className="flex flex-col items-center gap-2 animate-pulse">
              <div className="w-16 h-16 bg-[#0f2541] rounded-full flex items-center justify-center shadow-lg">
        ...
        <CardContent className="relative p-3 sm:p-5 md:p-6 lg:p-8 xl:p-10">
          ...
          <div className="text-center mb-3 sm:mb-4 md:mb-5 lg:mb-6">
            <div className="relative inline-block mb-2 sm:mb-3 md:mb-4">
              <div className="absolute inset-0 bg-[#0f2541]/35 rounded-full blur-lg scale-150" />
              <div className="relative w-9 h-9 sm:w-11 sm:h-11 md:w-14 md:h-14 bg-gradient-to-br from-[#0f2541] to-[#27467a] rounded-full flex items-center justify-center mx-auto shadow-lg">
            ...
            <p className="text-[10px] sm:text-xs md:text-sm text-foreground/70 font-lora">
              Your words will be part of Marielle&apos;s Moonlit Masquerade keepsake.
            </p>
          </div>
          ...
          >
            ...
            <div className="space-y-1.5 sm:space-y-2 md:space-y-3">
              <label className="block text-xs sm:text-sm md:text-base font-medium text-foreground font-lora flex items-center gap-1.5 sm:gap-2">
                <div className={`w-4 h-4 sm:w-5 sm:h-5 md:w-6 md:h-6 bg-gradient-to-br from-[#0f2541]/25 to-[#7db7ff]/25 rounded-full flex items-center justify-center transition-all duration-300 ${
                  focusedField === 'name' ? 'scale-110 bg-[#0f2541]/35' : ''
                }`}>
                  <Heart className="h-2.5 w-2.5 sm:h-3 sm:w-3 md:h-4 md:w-4 text-[#9ac8ff]" />
                </div>
                Your Name
              </label>
              <div className="relative">
                <Input
                  ...
                  className={`message-form-input w-full border-2 rounded-xl py-2 sm:py-2.5 md:py-3 lg:py-3.5 px-3 sm:px-4 md:px-5 text-xs sm:text-sm md:text-base font-lora placeholder:italic transition-all duration-300 bg-white/85 backdrop-blur-sm shadow-sm hover:shadow-md focus:shadow-lg ${
                    focusedField === 'name' 
                      ? 'border-[#7db7ff] focus:border-[#7db7ff] focus:ring-4 focus:ring-[#7db7ff]/25 shadow-lg' 
                      : 'border-[#cfe7ff]/50 hover:border-[#7db7ff]/35'
                  }`}
                />
                {nameValue && (
            ...
            <div className="space-y-1.5 sm:space-y-2 md:space-y-3">
              <div className="flex items-center justify-between">
                <label className="block text-xs sm:text-sm md:text-base font-medium text-foreground font-lora flex items-center gap-1.5 sm:gap-2">
                  <div className={`w-4 h-4 sm:w-5 sm:h-5 md:w-6 md:h-6 bg-gradient-to-br from-[#0f2541]/25 to-[#7db7ff]/25 rounded-full flex items-center justify-center transition-all duration-300 ${
                    focusedField === 'message' ? 'scale-110 bg-[#0f2541]/35' : ''
                  }`}>
                    <MessageCircle className="h-2.5 w-2.5 sm:h-3 sm:w-3 md:h-4 md:w-4 text-[#9ac8ff]" />
                  </div>
                  Your Message
                </label>
                {messageValue && (
              ...
              <div className="relative">
                <Textarea
                  ...
                  onBlur={() => setFocusedField(null)}
                  placeholder="Leave a wish for Marielle—share your words that will glow with her debut 💫"
            ...
            <Button
              ...
              className="w-full text-white py-2 sm:py-2.5 md:py-3 lg:py-3.5 px-4 sm:px-5 md:px-6 lg:px-7 rounded-xl text-xs sm:text-sm md:text-base font-lora font-semibold backdrop-blur-sm transition-all duration-300 hover:scale-[1.02] active:scale-[0.98] disabled:opacity-70 disabled:cursor-not-allowed disabled:transform-none relative overflow-hidden group border border-[#7db7ff]/70 bg-gradient-to-r from-[#0f2541] via-[#1e3a67] to-[#27467a]"
              style={{ 
                boxShadow: "0 6px 24px rgba(0,0,0,0.45), 0 2px 12px rgba(39,70,122,0.65)",
              }}
              onMouseEnter={(e) => {
                if (!e.currentTarget.disabled) {
                  e.currentTarget.style.boxShadow = "0 10px 30px rgba(0,0,0,0.6), 0 4px 14px rgba(39,70,122,0.75)";
                }
              }}
              onMouseLeave={(e) => {
                e.currentTarget.style.boxShadow = "0 6px 24px rgba(0,0,0,0.45), 0 2px 12px rgba(39,70,122,0.65)";
              }}
            >
              ...
              {isSubmitting ? (
                ...
                <span className="flex items-center justify-center gap-2 relative z-10">
                  <Send className="h-4 w-4 sm:h-5 sm:w-5 text-[#cfe7ff]" />
                  Send to Marielle
                </span>
              )}
            </Button>
...
export function Messages() {
  ...
  const fetchMessages = useCallback(() => {
    setLoading(true)
    fetch(
      "https://script.google.com/macros/s/AKfycbwDtCQhDgJcMeMI3caniLJk2G-XEHw5tvdEMajdDAC-sp9i_JVTkx81AZdsQqEdIdpC/exec"
    )
      .then((res) => res.json())
      .then((data) => {
    ...
    >
      <div className="relative max-w-6xl mx-auto px-2 sm:px-4 md:px-6 lg:px-8">
        ...
        <div className="text-center mb-4 sm:mb-6 md:mb-8 lg:mb-10">
          ...
          >
              Leave a heartfelt note for Marielle—every wish becomes part of her debut story under the moonlight.
          </p>
        </div>
        ...
        <div className="relative max-w-4xl mx-auto">
          <div className="text-center mb-4 sm:mb-6 md:mb-8">
            <div className="relative inline-block mb-3 sm:mb-4 md:mb-6">
              <div className="absolute inset-0 bg-gradient-to-r from-[#0f2541]/55 via-[#27467a]/35 to-[#7db7ff]/30 rounded-full blur-xl scale-150 animate-pulse-slow" />
              <div className="relative w-8 h-8 sm:w-10 sm:h-10 md:w-14 md:h-14 bg-gradient-to-br from-[#0f2541] via-[#1e3a67] to-[#7db7ff] rounded-full flex items-center justify-center mx-auto shadow-lg hover:scale-110 transition-transform duration-300">
                <MessageCircle className="h-4 w-4 sm:h-6 sm:w-6 md:h-8 md:w-8 text-white" />
              </div>
              {/* Outer glow ring */}
              <div className="absolute -inset-2 rounded-full bg-gradient-to-br from-[#0f2541]/35 via-[#27467a]/25 to-[#7db7ff]/25 blur-md opacity-0 hover:opacity-100 transition-opacity duration-300" />
            </div>
            ...
            <p className="text-xs sm:text-sm md:text-base text-white font-lora max-w-2xl mx-auto px-2 sm:px-4">
              Night-sky notes from family and friends lighting up Marielle&apos;s debut
            </p>
          </div>
